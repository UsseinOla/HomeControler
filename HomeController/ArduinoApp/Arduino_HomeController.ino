...
rtc.setDate(6, 20, 2017);   // Set the date to October 3th, 2010
  ...
  digitalWrite(4,HIGH);  
if (!SD.begin(chipSelect)) {// chipselect cs=pin 53
...
void loop(){
 // Serial.println((analogRead(HUMPin)));
  // Serial.println("2   "+String((analogRead(HUMPin)*4.9)/1023.0));
  //*****************************Raindrops sensor***************************
...
void wifisent(){
 ...
 Serial.println(String(countLine)+")"+myFile.readStringUntil('\n'));
    ...
    Serial.println("sdcard;"+String(countLine)+";"+sdcardReadArray[i]);}
   //Serial.println("sdcard;"+sdcardReadArray[i]);}
    countLine=0;*/
  else {
